				...
				for field in globle_excel_report_id.field_ids:
					...
					if field.field_id.ttype=="date":
						if record:
							...
							record = record.strftime(user_date_format or '%d/%m/%Y')
						else:
							record = ''
	...
	def detailed_excel_report(self):
		...
		for record in self._context.get('active_ids'):
			...
			for header_value in globle_excel_report_id.field_ids:
				...
				if header_value.field_id.ttype=="date":
						if record_data:
							user_date_format = self.env['res.lang']._lang_get(self.env.user.lang).date_format
							record_data = record_data.strftime(user_date_format or '%d/%m/%Y')
						else:
							record_data = ''
			...
			for submodel in submodel_record:
				for sub_model_field_value in globle_excel_report_id.sub_model_field_ids:
					...
					if sub_model_field_value.field_id.ttype=="many2one":
							record=record.name
					if sub_model_field_value.field_id.ttype=="date":
							if record:
								user_date_format = self.env['res.lang']._lang_get(self.env.user.lang).date_format
								record = record.strftime(user_date_format or '%d/%m/%Y')
							else:
